...
#https://app.datacamp.com/workspace/preview?_tag=rdocs&rdocsPath=packages%2FROSE%2Fversions%2F0.0-4%2Ftopics%#2Fovun.sample&utm_source=r-docs&utm_medium=docs&utm_term=ovun.sample&utm_content=run_example_in_workspace

datos_entrenamiento_balanceados <- ovun.sample(smoking~., data=datos_entrenamiento, 
                                  p=0.50, seed=1, 
                                  method="over")$data
```
...
```
```{r}
#TOTAL DE DATOS BALANCEADOS POR UNDERSAMPLING

summary(datos_entrenamiento_balanceados$smoking)
...
table(dataset_filtrado$smoking)

# Realizar submuestreo
set.seed(2023)  
datos_seleccionados <- dataset_filtrado %>%
  group_by(smoking) %>%
  sample_n(23543)

# Verificar el nuevo conteo
table(datos_seleccionados$smoking)
